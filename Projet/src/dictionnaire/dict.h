...
dict_t dict_new(dict_index_t max_elements, dict_index_t nb_autoindex);
...
dict_error_t dict_search (dict_t d, dict_index_t prefix, dict_char_t value, dict_index_t *index);

// Inserer le noeud <prefix,value> dans le dictionnaire d
// Resultat index : index du noeud deja present ou nouvellement insere

dict_error_t dict_insert (dict_t d, dict_index_t prefix, dict_char_t value, dict_index_t *index);

// Lire les informations associees a un noeud :
// + index du prefixe
// + valeur du suffixe
dict_error_t dict_index_content (dict_t d, dict_index_t index, dict_char_t *value, dict_index_t *prefix_index);
...
dict_error_t dict_info (dict_t d, dict_index_t *last_allocated, dict_index_t *max_index);
...
dict_error_t dict_apply_reverse (dict_t dict, dict_index_t index, dict_apply_func_t f);

// Obtention de caractere initial + longueur de la chaine d'un noeud
dict_error_t dict_firstchar_length (dict_t dict, dict_index_t index, dict_char_t *carac, unsigned int *length);

// Retourne dans s un tableau de taille *s_size alloué dynamiquement
// (l'appelante devra appeler free pour le libérer)
dict_error_t dict_get_mallocedstring (dict_t dict, dict_index_t index, unsigned int *s_size, dict_char_t *s[]);
